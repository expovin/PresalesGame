...
class Presales  {
    ...
    getTimePerQuarter(){ return (this.timePerQuarter)}
    getFeature(){ return(features[Math.floor(Math.random() * trends.length)]) }    

    changeSatisfactionalLevel(percentage, absolute){
        if(percentage === null )
            this.satisfactionLevel +=absolute;
        else
            this.satisfactionLevel *= (1+percentage);
    ...
    resign(){
        ...
        if(foundBetter > this.satisfactionLevel){
            this.isEmployed=false;
            const resignationLetterTo = this.person.employedBy;
            this.person.employedBy="on the market";
            this.person.satisfactionLevel=initSatisfactionLevel;
