...
class Market {
    ...
    getCourse(companyID, personID, marketTrend, feature, money, hours, quantityToIncrease){
        ...
        this.Companies[companyID].payHours(hours);
        this.Companies[companyID].payMoney(money);

        if(marketTrend !== undefined){
            console.log("Sono in marketTrend aumento di  : ",quantityToIncrease);
            this.People[personID].increaseTrend(marketTrend, quantityToIncrease);
            this.People[personID].addCourse({marketTrend : marketTrend, hours : hours})
        }
            
        
        if(feature !== undefined){
            console.log("Sono in feature aumento di  : ",quantityToIncrease);
            this.People[personID].increaseFeature(feature, quantityToIncrease);
            this.People[personID].addCourse({feature : feature, hours : hours})
        }
    }
