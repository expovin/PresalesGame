...
function dockConfig() {
  ...
  var _settings$dock = settings.dock,
      dock = _settings$dock === void 0 ? '' : _settings$dock,
      _settings$displayOrde = settings.displayOrder,
      displayOrder = _settings$displayOrde === void 0 ? 0 : _settings$displayOrde,
      _settings$prioOrder = settings.prioOrder,
      prioOrder = _settings$prioOrder === void 0 ? 0 : _settings$prioOrder,
      _settings$requiredSiz = settings.requiredSize,
      requiredSize = _settings$requiredSiz === void 0 ? function () {
    return 0;
  } : _settings$requiredSiz,
      minimumLayoutMode = settings.minimumLayoutMode,
      _settings$show = settings.show,
      show = _settings$show === void 0 ? true : _settings$show;
  /**
   * @private
   */
...
function validateComponent(component) {
  ...
  expectedProperties.forEach(function (p) {
    if (typeof component[p] !== 'function') {
      throw new Error("Component is missing required function \"".concat(p, "\""));
    }
  });
}
...
function reduceLayoutRect(logicalContainerRect, components, hiddenComponents, settings) {
  ...
  components.length = 0;
  components.push.apply(components, _toConsumableArray$1(filteredUnsortedComps));
...
function boundingBox(rects) {
  ...
  var points = (_ref = []).concat.apply(_ref, _toConsumableArray$1(rects.map(rectToPoints)));
...
function dockLayout(initialSettings) {
  ...
  docker.addComponent = function addComponent(component, key) {
    ...
    docker.removeComponent(component); // component.dockConfig can be undefined, a function or an object:
  ...
  docker.layout = function layout(container) {
    var _resolveLayout = resolveLayout(container, settings),
        _resolveLayout2 = _slicedToArray(_resolveLayout, 2),
...
function getOrCreateFormatter(v, formatters, data, deps) {
  ...
  if (typeof v === 'string' && formatters[v]) {
    // return by name
    f = formatters[v];
  } else if (_typeof(v) === 'object' && 'formatter' in v && formatters[v.formatter]) {
    // return by { formatter: "name" }
    f = formatters[v.formatter];
  } else if (_typeof(v) === 'object' && 'type' in v && formatters[v.type]) {
    // return by { formatter: "name" }
    f = formatters[v.type];
  }
...
function ticksByValue(_ref4) {
  ...
  return values.sort(function (a, b) {
    ...
    return {
      ...
      isMinor: false,
      start: isObject(v) && !isNaN(v.start) ? clamp(scale(v.start)) : position,
...
function getOrCreateScale(v, scales, d, deps) {
  ...
  if (typeof v === 'string' && scales[v]) {
    // return by name
    s = scales[v];
  } else if (_typeof(v) === 'object' && 'scale' in v && scales[v.scale]) {
...
function prepareContext(ctx, definition, opts) {
  ...
  var settings = opts.settings,
      ...
      update = opts.update,
      _DO_NOT_USE_getInfo = opts._DO_NOT_USE_getInfo; // TODO add setters and log warnings / errors to console
...
function componentFactory(definition) {
  ...
  fn.dockConfig = _dockConfig; // Set new settings - will trigger mapping of data and creation of scale / formatter.
...
function chartFn(definition, context) {
  ...
  var set = function set(_data, _settings) {
    ...
    currentScales = builder$1(scales, {
      dataset: dataset,
    ...
    currentFormatters = builder(formatters, {
      dataset: dataset,
      collection: dataCollection
    }, {
  ...
  var componentsFromPoint = function componentsFromPoint(p) {
    ...
    visibleComponents.forEach(function (c) {
      var r = c.instance.getRect(); // Do test on physical rect
  ...
  instance.scale = function scale(v) {
    return getOrCreateScale(v, currentScales, {
      dataset: dataset,
      collection: dataCollection
    }, {
      scale: registries.scale,
  ...
  instance.formatter = function formatter(v) {
    return getOrCreateFormatter(v, currentFormatters, {
      dataset: dataset,
      collection: dataCollection
    }, {
      formatter: registries.formatter,
...
function box(_ref) {
  ...
  return extend({}, item.box, (_extend = {
    type: 'rect'
  }, _defineProperty(_extend, x, (boxPadding + item.major) * calcwidth), _defineProperty(_extend, y, actualLow), _defineProperty(_extend, height, actualDiff), _defineProperty(_extend, width, boxWidth * calcwidth), _defineProperty(_extend, "data", item.data || {}), _defineProperty(_extend, "collider", {
    type: null
  }), _extend));
}
function verticalLine(_ref2) {
  ...
  return extend({}, item.line, (_extend2 = {
    type: 'line'
  }, _defineProperty(_extend2, y2, Math.floor(from * calcheight)), _defineProperty(_extend2, x1, boxCenter * calcwidth), _defineProperty(_extend2, y1, Math.floor(to * calcheight)), _defineProperty(_extend2, x2, boxCenter * calcwidth), _defineProperty(_extend2, "data", item.data || {}), _defineProperty(_extend2, "collider", {
    type: null
  }), _extend2));
}
function horizontalLine(_ref3) {
  ...
  return extend({
    type: 'line'
  }, item[key], (_extend3 = {}, _defineProperty(_extend3, y1, Math.floor(position * calcheight)), _defineProperty(_extend3, x1, (boxCenter - halfWidth) * calcwidth), _defineProperty(_extend3, y2, Math.floor(position * calcheight)), _defineProperty(_extend3, x2, (boxCenter + halfWidth) * calcwidth), _defineProperty(_extend3, "r", halfWidth * calcwidth), _defineProperty(_extend3, "cx", boxCenter * calcwidth), _defineProperty(_extend3, "cy", position * calcheight), _defineProperty(_extend3, "width", width * calcwidth), _defineProperty(_extend3, "data", item.data || {}), _defineProperty(_extend3, "collider", {
    type: null
  }), _extend3));
}
...
function buildShapes(_ref4) {
  ...
  var _loop = function _loop(i, len) {
    ...
    var boxWidth = getBoxWidth(bandwidth, item, maxMajorWidth);
...
var gridLineComponent = {
  ...
  render: function render() {
    ...
    var style = {}; // Loop through all X and Y lines

    _toConsumableArray$1(this.lines.x).concat(_toConsumableArray$1(this.lines.y)).forEach(function (p) {
      style = p.isMinor ? _this.settings.minorTicks : _this.settings.ticks; // If the style's show is falsy, don't renderer this item (to respect axis settings).
...
function buildRange(_ref4) {
  ...
  if (bubbles && bubbles.show) {
    ...
    els.push(buildBubble({
      ...
      label: "".concat(state.format(start < end ? vStart : vEnd, range)),
      pos: top,
      state: state
    }));
    els.push(buildBubble({
      ...
      label: "".concat(state.format(start < end ? vEnd : vStart, range)),
      pos: bottom,
      state: state
    }));
  }
...
function nodes(state) {
  ...
  var els = [];
  var isVertical = state.direction === VERTICAL; // add all other ranges
...
function render$4(data, placement, _ref) {
...
var component$3 = {
  ...
  appendTo: function appendTo() {
    if (this.props.appendTo) {
      ...
      var bounds = this.state.targetElement.getBoundingClientRect();
      var size = {
        width: bounds.width,
        height: bounds.height,
  ...
  invokeRenderer: function invokeRenderer(nodes) {
    ...
    var pseudoElement = render$4(items, {
      style: {
        left: '0px',
        top: '0px',
        visibility: 'hidden'
    ...
    this.state.tooltipElm = render$4(items, pos, this);
  }
};
...
function (_Container) {
  ...
  function Stage(dpi) {
    ...
    _this._stage = _assertThisInitialized(_assertThisInitialized(_this));
...
function measureTextWidth(_ref) {
  ...
  g.font = "".concat(fontSize, " ").concat(fontFamily);
...
function measureText(_ref3) {
  ...
  // eslint-disable-line import/prefer-default-export
  var w = measureTextWidth({
    text: text,
    fontSize: fontSize,
    fontFamily: fontFamily
  });
  var h = measureTextHeight({
    fontSize: fontSize,
    fontFamily: fontFamily
  });
  return {
...
function renderer() {
  ...
  canvasRenderer.render = function (shapes) {
    ...
    if (hasChangedRect) {
      el.style.left = "".concat(Math.round(rect.margin.left + rect.x * scaleX), "px");
      el.style.top = "".concat(Math.round(rect.margin.top + rect.y * scaleY), "px");
      el.style.width = "".concat(Math.round(rect.width * scaleX), "px");
      el.style.height = "".concat(Math.round(rect.height * scaleY), "px");
...
register$1('rect', render$5);
register$1('circle', render$6);
register$1('line', render$7);
register$1('path', render$9);
register$1('text', render$8);
...
function checkGradient() {
  ...
  if (gradientHashMap[gradientHash] !== undefined) {
    ...
    gradient.id = "picasso-gradient-".concat(gradientHash);
    gradient.children = stops.map(function (_ref) {
      ...
      return {
        ...
        style: "stop-color:".concat(color, ";stop-opacity:").concat(opacity || 1)
      };
    });
...
function renderer$1() {
  ...
  svg.render = function (nodes) {
    ...
    var sceneContainer = {
      type: 'container',
      children: Array.isArray(nodes) ? _toConsumableArray$1(nodes).concat([createDefsNode()]) : nodes
    };
    ...
    var newScene = sceneFn({
      items: [sceneContainer],
      on: {
        create: [onGradient, onLineBreak(svg.measureText), injectTextBoundsFn(svg)]
      }
    });
    var hasChangedScene = scene$$1 ? !newScene.equals(scene$$1) : true;
    var doRender = hasChangedRect || hasChangedScene;
...
function render$a(vnode, parent, merge) {
...
function renderer$2() {
  ...
  dom.render = function (nodes) {
    ...
    var scaleY = rect.scaleRatio.y;
    el.style.left = "".concat(Math.round(rect.margin.left + rect.x * scaleX), "px");
    el.style.top = "".concat(Math.round(rect.margin.left + rect.y * scaleY), "px");
    el.style.width = "".concat(Math.round(rect.width * scaleX), "px");
    el.style.height = "".concat(Math.round(rect.height * scaleY), "px");
    var vNode;
